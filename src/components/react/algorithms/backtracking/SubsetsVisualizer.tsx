...
function computeSteps(): TreeStep[] {
  ...
  const treeNodes: TreeNode[] = [
    ...
    { id: '12', label: '[1,2]', x: 50, y: 135, children: ['123'] },
    { id: 'skip12', label: '[1]', x: 150, y: 135, children: ['13'] },
    { id: '2only', label: '[2]', x: 250, y: 135, children: ['23'] },
    { id: 'skip2only', label: '[]', x: 350, y: 135, children: ['3only'] },
    { id: '123', label: '[1,2,3]', x: 30, y: 200, children: [] },
    { id: '13', label: '[1,3]', x: 120, y: 200, children: [] },
    { id: '23', label: '[2,3]', x: 230, y: 200, children: [] },
    { id: '3only', label: '[3]', x: 340, y: 200, children: [] },
  ];
  ...
  const leafIds = new Set(['123', '13', '23', '3only', 'skip12', 'skip2only']);

  for (const path of paths) {
    const isLeaf = leafIds.has(path.id);
    const highlights: Record<string, string> = { [path.id]: 'current' };
    for (const r of results) {
      const matchingNode = paths.find((p) => p.subset === r && leafIds.has(p.id));
      if (matchingNode) highlights[matchingNode.id] = 'visited';
    }

    if (isLeaf && !results.includes(path.subset)) {
  ...
  steps.push({
    ...
    visitOrder: allSubsets,
    description: `${allSubsets.length} subsets encontrados!`,
...
Esse padrão é a base para resolver permutações, combinações, N-Queens, Sudoku, e qualquer problema que explore um espaço de busca com restrições. A otimização chave é podar ramos que não podem levar a soluções válidas (pruning).`}
    ...
    >
      <TreeVisualizer step={step} color="#f97316" height={240} />
    </AlgorithmShell>
  );
}
